...
<head>
  ...
  <title>General Physician Section</title>
  <style>
    body {
      background: #ffffff;
      margin: 0;
...
<section class="doctor-specialty">
  ...
  <div class="doctor-cards">
    <?php
      // Fetch doctors with specialization 'General Physician'
      $sql = "SELECT * FROM doctors WHERE specialization = 'General Physician'";
      ...
      if ($result && $result->num_rows > 0) {
          while ($row = $result->fetch_assoc()) {
              ...
              echo '</ul>
                    <a class="appointment-btn" href="../appointment.html?doctor=' . urlencode($row["name"]) . '">Book Appointment</a>
                    </div>
                    <div class="rating">' . htmlspecialchars($row["rating"]) . '</div>
                  </div>';
          }
      } else {
          echo "<p>No general physicians found in the database.</p>";
      }
